...
public class SceneController : MonoBehaviour
{
    ...
    [SerializeField] private LoadingOverlay loadingScreen;
    private Dictionary<string, string> loadedSceneSlot = new Dictionary<string, string>();
    ...
    void Awake()
    {
        ...
        Instance = this;
        DontDestroyOnLoad(gameObject);   // si tu veux que le SceneController survive aux changements de scène
    }

    // API
    public SceneTransitionPlan NewTransition()   // (j’ai corrigé l’orthographe)
    {
    ...
    private IEnumerator ChangeSceneRoutine(SceneTransitionPlan plan)
    {
        ...
        {
            if (loadedSceneSlot.ContainsKey(kvp.Key))
            {
    ...
    private IEnumerator LoadAdditiveRoutine(string slotKey, string sceneName, bool setActive)
    {
        ...
        loadedSceneSlot[slotKey] = sceneName;
    }

    private IEnumerator UnloadSceneRoutine(string slotKey)
    {
        if (!loadedSceneSlot.TryGetValue(slotKey, out string sceneName)) yield break;
        ...
        if (unloadOp != null)
        {
            ...
            loadedSceneSlot.Remove(slotKey);
        }
    }
